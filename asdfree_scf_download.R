...
years.to.download <- c( 1983 , 1986 , 1989 , 1992 , 1995 , 1998 , 2001 , 2004 , 2007 , 2009 , 2010 , 2013 , 2016)
...
library('haven') 	# load foreign package (converts data files into R)
...
downloads <-
	data.frame(
		year = 
			c( 1989 , 1992 , 1995 , 1998 , 2001 , 2004 , 2007 , 2009 , 2010  , 2013 , 2016) ,
		main = 
			c( 'scf89s' , 'scf92s' , 'scf95s' , 'scf98s' , 'scf01s' , 'scf2004s' , 'scf2007s' , 'scf2009ps' , 'scf2010s' , 'scf2013s' , 'scf2016s' ) ,
		extract = 
			c( 'scfp1989s' , 'scfp1992s' , 'scfp1995s' , 'scfp1998s' , 'scfp2001s' , 'scfp2004s' , 'scfp2007s' , 'rscfp2009panels' , 'scfp2010s' , 'scfp2013s' , 'scfp2016s' ) ,
		rw = 
    	c( 'scf89rw1s' , 'scf92rw1s' , 'scf95rw1s' , 'scf98rw1s' , 'scf2001rw1s' , 'scf2004rw1s' , 'scf2007rw1s' , 'scf2009prw1s' , 'scf2010rw1s' , 'scf2013rw1s' , 'scf2016rw1s')
			#c( 'scf89rw1s' , 'scf92rw1s' , 'scf95rw1s' , '1998_scf98rw1s' , 'scf2001rw1s' , '2004_scf2004rw1s' , '2007_scf2007rw1s' , 'scf2009prw1s' , 'scf2010rw1s' , 'scf2013rw1s' , 'scf2016rw1s')
...
read.scf <-
	...
	function( zip.fn , http.path = "http://www.federalreserve.gov/econres/files/" ){
...
for ( i in seq( nrow( downloads ) ) ){
  ...
  # If file is already downloaded skip to next iteration
  if (file.exists(file.to.save)) next;
	...
	# download and import the replicate weights
  ...
  # Use xz for best compression ratio. Change to lzip if available in a future.
	save( imp1 , imp2 , imp3 , imp4 , imp5 , rw , file = file.to.save, compress = 'xz' )
